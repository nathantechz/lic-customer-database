...
def create_database():
    ...
    # Insert known agents from config if available
    ...
    if config_path.exists():
        try:
            ...
            agents_data = []
            for agent_code, agent_info in config.get('agents', {}).items():
                agents_data.append((
                    agent_code,
                    agent_info.get('agent_name', ''),
                    agent_info.get('branch_code', ''),
            ...
            print(f"✓ Loaded {len(agents_data)} agents from config")
            
        except Exception as e:
            print(f"⚠️  Could not load agents from config: {e}")
    else:
        # Insert default agents if no config file
        default_agents = [
            ('0089174N', 'Your Son', '74N', 'son', '', '', True),
